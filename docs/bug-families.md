...
- **截断 tool-call 参数子族**：[#3234](https://github.com/deepseek-ai/deepseek-harness/discussions/3234)（流式 arguments 截断 JSON 落盘 → 每回合 400 INVALID_REQUEST，会话永久砖化）——assembler.ts:72 拼接无校验 + tool-calls.ts:104-108 吞错返回 raw（工具带垃圾参数真实执行！）+ serialize.ts:82 原样重发 + apiproxy 裸 parse；修复=BlockAssembler finalize 校验并拒绝无效调用（根治）+ serialize safeArguments 兜底 + load() 可确定性修复则愈合
